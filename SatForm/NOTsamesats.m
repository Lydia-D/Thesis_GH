%% L Drabsch
% 20/5/17
% if not all receivers have the same satellites in view
...
allSats = VisibleSat(:,:)';
rec_index_sats{1} = [1,2,3,4];
rec_index_sats{2} = [1,2,5,6];
rec_index_sats{3} = [3,4,5,6];
%rec_index_sats{4} = allSats([7,8,9,10],:);
...
range_error = all_range+1*rand(size(all_range)) - c*clockbias_rec*ones(1,numSat) + c*ones(numRec,1)*sat_error_time;
...
tic
for i = 1:numRec
    [Rec_abs(:,i),~,calc_clockbias(i,1)] = convergance([X_alpha_ECEF;0],allSats',range_error(i,:)');
end
toc
...
tic
[Loc_lin,clockbias] = alloptimise_varsat(range_error,allSats, Rec_abs,rec_index_sats);
toc
Loc_lin
